...
const project = (point: ProjectionPoint): { x: number; y: number } => {
  projectedPoint.set(point.x, point.y, point.z).project(instance);

  const x = projectedPoint.x * threeSizes.width * 0.5;
  const y = -projectedPoint.y * threeSizes.height * 0.5;
